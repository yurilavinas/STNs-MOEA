...
read.data.continuous <- function(inpath, algorithm, problem) {
  ...
  all.data = read.table(file = filename,
                        header = T,
                        sep = ',')
...
generate.vector.data.continuous <-
  function(all.data,
           minP,
           od = 3,
           width = 8) {
    ...
    for (j in 1:max(all.data$run)) {
      ...
      for (i in 0:max.iter) {
        ...
        #filtering data of the main iteration - where the search 'come from'
        filtered.data1 = filter(algorithm.data, iter == i)
        #filtering data that contains violated values
        # filtered.data1 = filtered.data1 %>% filter(if_any(starts_with("Y"), ~ . 
        #                                                     < 1e10))
        {}
        # finding the representatives solutions
        representatives1 <-
        ...
        solution1 = format(as.hexmode(apply(round(X1, od) * 10 ^ od, 2, as.integer)), width =
                             width)
        ...
        filtered.data2 = filter(algorithm.data, iter == i + 1)
        #filtering data that contains violated values
        # filtered.data2 = filtered.data2 %>% filter(if_any(starts_with("Y"), ~ . 
        #                                                   < 1e10))
        ...
        # concatenating the dimensions of the solutions from the whole data
        # creating the hyperspace partition
        solution2 = format(as.hexmode(apply(round(X2, od) * 10 ^ od, 2 , as.integer)), width =
                             width)
